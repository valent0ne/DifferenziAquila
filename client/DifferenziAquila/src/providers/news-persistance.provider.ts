...
export class NewsPersistanceProvider implements NewsPersistanceInterface {
  ...
  getAmountFromStorage(): Promise<number> {
    return new Promise(resolve => {
      this._storage.get(STORAGE_KEYS.NEWS)
        .then((n) => {
          console.log("[NewsPersistanceProvider] amount from storage: " + n.length);
          resolve(n.length);
        }).catch(() => {
        console.log("[NewsPersistanceProvider] no news in storage");
        resolve(0);
      });
    });
  }
      ...
      .then((res: Response) => {
        ...
        if (json.result) {
          ...
          this.save(out).then(() => {
            ...
            console.log("[NewsPersistanceProvider] - catch n news storage ");
            reject();
          })
        } else {
